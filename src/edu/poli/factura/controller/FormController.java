...
import edu.poli.factura.model.Alimento;
import edu.poli.factura.model.Almacen;
import edu.poli.factura.model.Aseo;
import edu.poli.factura.model.Cheque;
import edu.poli.factura.model.Consumidor;
import edu.poli.factura.model.Efectivo;
import edu.poli.factura.model.Electronico;
import edu.poli.factura.model.Factura;
import edu.poli.factura.model.FormaDePago;
import edu.poli.factura.model.Producto;
import edu.poli.factura.model.Producto1;
import edu.poli.factura.model.Tarjeta;
...
conexiion = conexion.conectar();
    ...
    void save5(ActionEvent event) {
    	   ...
    	   /*
    	if (opt7.isSelected()) {
			Factura f=new Factura(Integer.parseInt(txt31.getText()), date1.getValue().toString(), a, c, productos, pago);
			//txtfact.setText(f.toString());
		 If.setText(f.toString()); 
       ...
       void buscar(ActionEvent event) {
    	   conexiion = conexion.conectar();
          	try {
      			...
      			if (resultSet.next()) {
      				//fechafactura.setText(resultSet.getString("fecha"));
       ...
       void eliminar(ActionEvent event) {
    	   Producto1 product= new Producto1("xdvxcv", "sdf", 23, 20, 30, "jj");
...
public static ObservableList<Producto1> getDataProducto(){
		...
		try {
			PreparedStatement ps = conn.prepareStatement("select p.codigo, p.nombre , p.precio, p.impuesto,  p.cantidad,  p.tipo,  from proyectobd.producto p inner join proyectobd.consumidor c on p.idConsumidor=c.idConsumidor inner join proyectobd.factura f on c.numero= f.numero where f.numero=?;");
			//ps.setString(1,txt100.getText());
